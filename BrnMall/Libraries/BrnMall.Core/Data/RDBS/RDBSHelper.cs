...
namespace BrnMall.Core
{
    ...
    public class RDBSHelper
    {
        ...
        private static string _executedetail;
        ...
        private static string GetExecuteDetail(string commandText, DateTime startTime, DateTime endTime, DbParameter[] commandParameters)
        {
            ...
            {
                StringBuilder paramdetails = new StringBuilder("<table width=\"960px\" style=\"border: 1px solid black;\" cellspacing=\"0\" cellpadding=\"0\" style=\"background: rgb(255, 255, 255) none repeat scroll 0%; margin-top: 10px; font-size: 12px; display: block; \">");
                paramdetails.AppendFormat("<tr><td>SQL:{0}</td></tr>", commandText);
                paramdetails.AppendFormat("<tr><td>Time:{0}</td></tr>", endTime.Subtract(startTime).TotalMilliseconds / 1000);
                foreach (DbParameter param in commandParameters)
                {
                    if (param == null)
                        continue;

                    paramdetails.AppendFormat("<tr><td>Name:{0}</td></tr>", param.ParameterName);
                    paramdetails.AppendFormat("<tr><td>Type:{0}</td></tr>", param.DbType);
                    paramdetails.AppendFormat("<tr><td>Value:{0}</td></tr>", param.Value);
                }
                return paramdetails.Append("</table>").ToString();
            }
            return string.Empty;
        }
#endif
