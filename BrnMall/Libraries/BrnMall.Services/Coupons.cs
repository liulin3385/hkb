...
namespace BrnMall.Services
{
    ...
    public class Coupons
    {
        ...
        {
            CouponTypeInfo couponTypeInfo = BrnMall.Core.BMACache.Get(CacheKeys.MALL_COUPONTYPE_INFO + couponTypeId) as CouponTypeInfo;
            if (couponTypeInfo == null)
            {
                couponTypeInfo = BrnMall.Data.Coupons.GetCouponTypeById(couponTypeId);
                BrnMall.Core.BMACache.Insert(CacheKeys.MALL_COUPONTYPE_INFO + couponTypeId, couponTypeInfo);
            }
        ...
        {
            return BrnMall.Data.Coupons.GetSendingPromotionCouponTypeList(storeId);
        }
        ...
        {
            return BrnMall.Data.Coupons.GetSendingCouponTypeList(storeId, nowTime);
        }
        ...
        {
            return BrnMall.Data.Coupons.GetUsingCouponTypeList(storeId, nowTime);
        }
        ...
        {
            BrnMall.Data.Coupons.CreateCoupon(couponInfo);
        }
        ...
        public static void DeleteCouponById(int[] idList)
        {
            if (idList != null && idList.Length > 0)
                BrnMall.Data.Coupons.DeleteCouponById(CommonHelper.IntArrayToString(idList));
        }
        ...
        {
            BrnMall.Data.Coupons.DeleteCouponById(idList);
        }
        ...
        {
            return BrnMall.Data.Coupons.IsExistCouponSN(couponSN);
        }
        ...
        {
            int sendCount = TypeHelper.ObjectToInt(BrnMall.Core.BMACache.Get(CacheKeys.MALL_COUPONTYPE_SENDCOUNT + couponTypeId), -1);
            if (sendCount < 0)
            {
                sendCount = BrnMall.Data.Coupons.GetSendCouponCount(couponTypeId);
                BrnMall.Core.BMACache.Insert(CacheKeys.MALL_COUPONTYPE_SENDCOUNT + couponTypeId, sendCount);
            }
        ...
        {
            return BrnMall.Data.Coupons.GetSendUserCouponCount(uid, couponTypeId);
        }
        ...
        {
            return BrnMall.Data.Coupons.GetTodaySendUserCouponCount(uid, couponTypeId, today);
        }
        ...
        {
            return BrnMall.Data.Coupons.GetCouponList(uid, 0);
        }
        ...
        {
            DataTable dt = BrnMall.Data.Coupons.GetCouponList(uid, 1);
            if (dt.Rows.Count < 1)
                return dt;
        ...
        {
            return BrnMall.Data.Coupons.GetCouponList(uid, 2);
        }
        ...
        {
            return BrnMall.Data.Coupons.GetCouponByCouponId(couponId);
        }
        ...
        {
            return BrnMall.Data.Coupons.GetCouponByCouponSN(couponSN);
        }
        ...
        /// <param name="couponInfo">优惠劵信息</param>
        /// <param name="isMut">是否为叠加使用</param>
        /// <param name="partUserInfo">用户信息</param>
        /// <param name="storeIdList">店铺id列表</param>
        /// <param name="orderProductList">订单商品列表</param>
        /// <returns></returns>
        public static int VerifyCoupon(out int result, CouponInfo couponInfo, bool isMut, PartUserInfo partUserInfo, List<int> storeIdList, List<OrderProductInfo> orderProductList)
        {
            result = 0;
            CouponTypeInfo couponTypeInfo = GetCouponTypeById(couponInfo.CouponTypeId);
            if (couponTypeInfo == null)
            {
                result = 1;
            }
            ...
            else if (couponTypeInfo.UseExpireTime < 1 && couponTypeInfo.UseStartTime > DateTime.Now)
            {
                result = 3;
            }
            else if (couponTypeInfo.UseExpireTime < 1 && couponTypeInfo.UseEndTime <= DateTime.Now)
            {
                result = 4;
            }
            ...
            else if (isMut || couponTypeInfo.UseMode == 1)
            {
                result = 6;
            }
            else if (couponTypeInfo.OrderAmountLower > (Orders.SumOrderProductAmount(orderProductList) - Orders.SumFullCut(orderProductList)))
            {
                result = 7;
            }
            else if (!storeIdList.Exists(x => x == couponTypeInfo.StoreId))
            {
                result = 8;
            }
            else
            {
                if (couponTypeInfo.LimitStoreCid > 0)
                {
                    ...
                    {
                        if (orderProductInfo.StoreId == couponTypeInfo.StoreId && orderProductInfo.Type == 0 && orderProductInfo.StoreCid != couponTypeInfo.LimitStoreCid)
                        {
                            result = 9;
                            return couponTypeInfo.LimitStoreCid;
                        }
                    }
                }
                ...
                if (couponTypeInfo.LimitProduct == 1)
                {
                    ...
                    if (!IsSameCouponType(couponTypeInfo.CouponTypeId, pidList))
                        result = 10;
                }
            }
            return 0;
        ...
        {
            BrnMall.Data.Coupons.UseCoupon(couponId, oid, time, ip);
        }
        ...
        {
            BrnMall.Data.Coupons.ActivateAndUseCoupon(couponId, uid, oid, time, ip);
        }
        ...
        {
            BrnMall.Data.Coupons.ActivateCoupon(couponId, uid, time, ip);
        }
        ...
        {
            BrnMall.Data.Coupons.ReturnUserOrderUseCoupons(oid);
        }
        ...
        {
            return BrnMall.Data.Coupons.GetUserOrderSendCouponList(oid);
        }
        ...
        {
            return BrnMall.Data.Coupons.IsExistCouponProduct(couponTypeId, pid);
        }
        ...
        {
            return BrnMall.Data.Coupons.IsSameCouponType(couponTypeId, pidList);
        }
    }
}
