...
namespace BrnMall.Services
{
    public class MallUtils
    {
        ...
        public static string SaveNewsEditorImage(HttpPostedFileBase image)
        {
            ...
            string name = "n_" + DateTime.Now.ToString("yyMMddHHmmssfffffff");
        ...
        /// <returns></returns>
        public static string SaveNewsEditorFile(HttpPostedFileBase file)
        {
            if (file == null)
                return "-1";
            ...
            string extension = Path.GetExtension(fileName);
            if (!CommonHelper.IsInArray(extension, mallConfig.UploadAttType))
                return "-2";

            int fileSize = file.ContentLength;
            if (fileSize > mallConfig.UploadAttSize)
                return "-3";

            string dirPath = IOHelper.GetMapPath("/Upload/News/File/");
        ...
        public static string SaveProductEditorImage(HttpPostedFileBase image)
        {
            ...
            string name = "pe_" + DateTime.Now.ToString("yyMMddHHmmssfffffff");
            string newFileName = name + extension;
        ...
        /// <param name="file">文件</param>
        /// <returns></returns>
        public static string SaveProductEditorFile(HttpPostedFileBase file)
        {
            if (file == null)
                return "-1";
            ...
            string dirPath = IOHelper.GetMapPath("/Upload/Product/Editor/File/");
            if (!Directory.Exists(dirPath))
                Directory.CreateDirectory(dirPath);
            file.SaveAs(dirPath + fileName);
        ...
        public static string SaveUploadFriendLinkLogo(HttpPostedFileBase friendLinkLogo)
        {
            ...
            string newFileName = string.Format("fr_{0}{1}", DateTime.Now.ToString("yyMMddHHmmssfffffff"), extension);//生成文件名
            string[] sizeList = StringHelper.SplitString(mallConfig.FriendLinkThumbSize);
        ...
        /// <returns></returns>
        public static string SaveUploadStoreLogo(HttpPostedFileBase storeLogo)
        {
            if (storeLogo == null)
                return "-1";
            ...
            string dirPath = IOHelper.GetMapPath("/Upload/Store/Logo/");
        ...
        /// <returns></returns>
        public static string SaveUploadStoreBanner(HttpPostedFileBase storeBanner)
        {
            if (storeBanner == null)
                return "-1";
            ...
            string dirPath = IOHelper.GetMapPath("/Upload/Store/Banner/");
