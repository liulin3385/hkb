...
namespace BrnMall.Services
{
    ...
    public class Helps
    {
        ...
        {
            List<HelpInfo> helpTree = BrnMall.Core.BMACache.Get(CacheKeys.MALL_HELP_LIST) as List<HelpInfo>;

            if (helpTree == null)
            {
                helpTree = new List<HelpInfo>();
                List<HelpInfo> helpList = BrnMall.Data.Helps.GetHelpList();

                CreateHelpTree(helpList, helpTree, 0);
                BrnMall.Core.BMACache.Insert(CacheKeys.MALL_HELP_LIST, helpTree);
            }
            return helpTree;
        }
