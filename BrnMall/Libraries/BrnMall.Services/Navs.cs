...
namespace BrnMall.Services
{
    ...
    public class Navs
    {
        ...
        {
            List<NavInfo> navTree = BrnMall.Core.BMACache.Get(CacheKeys.MALL_NAV_LIST) as List<NavInfo>;
            ...
            if (navTree == null)
            {
                ...
                BrnMall.Core.BMACache.Insert(CacheKeys.MALL_NAV_LIST, navTree);
            }
            return navTree;
        }
        ...
        {
            List<NavInfo> navList = BrnMall.Core.BMACache.Get(CacheKeys.MALL_NAV_MAINLIST) as List<NavInfo>;

            if (navList == null)
            {
                navList = new List<NavInfo>();
                foreach (NavInfo navInfo in BrnMall.Data.Navs.GetNavList())
                {
                    if (navInfo.Pid == 0)
                        navList.Add(navInfo);
                }
                BrnMall.Core.BMACache.Insert(CacheKeys.MALL_NAV_MAINLIST, navList);
            }
            return navList;
        }
