...
namespace BrnMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public void UpdatePVStat(string category, string value, int count)
        {
            DbParameter[] parms = {
        ...
        /// <returns></returns>
        public IDataReader GetLoginFailLogByIP(Int64 loginIP)
        {
            DbParameter[] parms = {
        ...
        public void AddLoginFailTimes(Int64 loginIP, DateTime loginTime)
        {
            DbParameter[] parms = {
									GenerateInParam("@loginip",SqlDbType.BigInt,8, loginIP),
        ...
        /// </summary>
        /// <param name="loginIP">登陆IP</param>
        public void DeleteLoginFailLogByIP(Int64 loginIP)
        {
            DbParameter[] parms = {
									GenerateInParam("@loginip",SqlDbType.BigInt,8, loginIP)
			                       };
        ...
        /// <returns></returns>
        public DataTable GetHomeBannerList(DateTime nowTime)
        {
            DbParameter[] param = {
                                    GenerateInParam("@nowtime",SqlDbType.DateTime,8,nowTime)
                                   };
            string commandText = string.Format("SELECT {1} FROM [{0}banners] WHERE [starttime]<=@nowtime AND [endtime]>@nowtime AND [isshow]=1 ORDER BY [displayorder] DESC,[id] DESC",
                                                RDBSHelper.RDBSTablePre,
                                                RDBSFields.BANNERS);
        ...
        public IDataReader AdminGetBannerList(int pageSize, int pageNumber)
        {
            ...
            if (pageNumber == 1)
            {
                commandText = string.Format("SELECT TOP {0} {2} FROM [{1}banners] ORDER BY [displayorder] DESC,[id] DESC",
                                            pageSize,
                                            RDBSHelper.RDBSTablePre,
                                            RDBSFields.BANNERS);
            ...
            {
                commandText = string.Format("SELECT TOP {0} {3} FROM [{1}banners] WHERE [id] NOT IN (SELECT TOP {2} [id] FROM [{1}banners] ORDER BY [displayorder] DESC,[id] DESC) ORDER BY [displayorder] DESC,[id] DESC",
                                            pageSize,
                                            RDBSHelper.RDBSTablePre,
                                            (pageNumber - 1) * pageSize,
                                            RDBSFields.BANNERS);
        ...
        public void CreateBanner(BannerInfo bannerInfo)
        {
            ...
            string commandText = String.Format("INSERT INTO [{0}banners]([starttime],[endtime],[isshow],[title],[img],[url],[displayorder]) VALUES(@starttime,@endtime,@isshow,@title,@img,@url,@displayorder)",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, param);
        }
        ...
        public void UpdateBanner(BannerInfo bannerInfo)
        {
            ...
            string commandText = String.Format("UPDATE [{0}banners] SET [starttime]=@starttime,[endtime]=@endtime,[isshow]=@isshow,[title]=@title,[img]=@img,[url]=@url,[displayorder]=@displayorder WHERE [id]=@id",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, param);
        }
        ...
        public void UpdateAdvert(AdvertInfo advertInfo)
        {
            ...
            string commandText = String.Format("UPDATE [{0}adverts] SET [clickcount]=@clickcount,[adposid]=@adposid,[state]=@state,[starttime]=@starttime,[endtime]=@endtime,[displayorder]=@displayorder,[type]=@type,[title]=@title,[url]=@url,[body]=@body WHERE [adid]=@adid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, param);
        }
        ...
        public DataTable AdminGetAdvertList(int pageSize, int pageNumber, int adPosId)
        {
            ...
            {
                if (adPosId < 1)
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] ORDER BY [adposid] DESC,[displayorder] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre);

                else
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] WHERE [adposid]={2} ORDER BY [adposid] DESC,[displayorder] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                adPosId);
            }
            ...
            {
                if (adPosId < 1)
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] WHERE [adid] NOT IN (SELECT TOP {2} [adid] FROM [{1}adverts] ORDER BY [adposid] DESC,[displayorder] DESC) ORDER BY [adposid] DESC,[displayorder] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize);
                else
                    commandText = string.Format("SELECT [temp1].[adid],[temp1].[clickcount],[temp1].[adposid],[temp1].[type],[temp1].[title] AS [atitle],[temp1].[starttime],[temp1].[endtime],[temp1].[state],[temp1].[displayorder],[temp2].[title] AS [aptitle] FROM (SELECT TOP {0} [adid],[clickcount],[adposid],[type],[title],[starttime],[endtime],[state],[displayorder] FROM [{1}adverts] WHERE [adposid]={3} AND [adid] NOT IN (SELECT TOP {2} [adid] FROM [{1}adverts] WHERE [adposid]={3} ORDER BY [adposid] DESC,[displayorder] DESC) ORDER BY [adposid] DESC,[displayorder] DESC) AS [temp1] LEFT JOIN [{1}advertpositions] AS [temp2] ON [temp1].[adposid]=[temp2].[adposid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                adPosId);
