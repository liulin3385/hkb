...
namespace BrnMall.RDBSStrategy.SqlServer
{
    ...
    public partial class RDBSStrategy : IRDBSStrategy
    {
        ...
        public int CreateStore(StoreInfo storeInfo)
        {
            ...
            string commandText = string.Format("INSERT INTO [{0}stores]([state],[name],[regionid],[storerid],[storeiid],[logo],[createtime],[mobile],[phone],[qq],[ww],[honesties],[stateendtime],[theme],[banner],[announcement],[description]) VALUES(@state,@name,@regionid,@storerid,@storeiid,@logo,@createtime,@mobile,@phone,@qq,@ww,@honesties,@stateendtime,@theme,@banner,@announcement,@description);SELECT SCOPE_IDENTITY();",
                                                RDBSHelper.RDBSTablePre);
        ...
        public void UpdateStore(StoreInfo storeInfo)
        {
            ...
            string commandText = string.Format("UPDATE [{0}stores] SET [state]=@state,[name]=@name,[regionid]=@regionid,[storerid]=@storerid,[storeiid]=@storeiid,[logo]=@logo,[createtime]=@createtime,[mobile]=@mobile,[phone]=@phone,[qq]=@qq,[ww]=@ww,[honesties]=@honesties,[stateendtime]=@stateendtime,[theme]=@theme,[banner]=@banner,[announcement]=@announcement,[description]=@description WHERE [storeid]=@storeid",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        /// <returns></returns>
        public DataTable AdminGetStoreList(int pageSize, int pageNumber, string condition, string sort)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[storeid],[temp1].[state],[temp1].[name],[temp1].[storerid],[temp1].[createtime],[temp2].[title],[temp3].[uid] FROM (SELECT TOP {0} [storeid],[state],[name],[storerid],[createtime] FROM [{1}stores] ORDER BY {2}) AS [temp1] LEFT JOIN [{1}storeranks] AS [temp2] ON [temp1].[storerid]=[temp2].[storerid] LEFT JOIN [{1}users] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                sort);

                else
                    commandText = string.Format("SELECT [temp1].[storeid],[temp1].[state],[temp1].[name],[temp1].[storerid],[temp1].[createtime],[temp2].[title],[temp3].[uid] FROM (SELECT TOP {0} [storeid],[state],[name],[storerid],[createtime] FROM [{1}stores] WHERE {2} ORDER BY {3}) AS [temp1] LEFT JOIN [{1}storeranks] AS [temp2] ON [temp1].[storerid]=[temp2].[storerid] LEFT JOIN [{1}users] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                condition,
                                                sort);
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT [temp1].[storeid],[temp1].[state],[temp1].[name],[temp1].[storerid],[temp1].[createtime],[temp2].[title],[temp3].[uid] FROM (SELECT TOP {0} [storeid],[state],[name],[storerid],[createtime] FROM [{1}stores] WHERE [storeid] NOT IN (SELECT TOP {2} [storeid] FROM [{1}stores] ORDER BY {3}) ORDER BY {3}) AS [temp1] LEFT JOIN [{1}storeranks] AS [temp2] ON [temp1].[storerid]=[temp2].[storerid] LEFT JOIN [{1}users] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                sort);
                else
                    commandText = string.Format("SELECT [temp1].[storeid],[temp1].[state],[temp1].[name],[temp1].[storerid],[temp1].[createtime],[temp2].[title],[temp3].[uid] FROM (SELECT TOP {0} [storeid],[state],[name],[storerid],[createtime] FROM [{1}stores] WHERE [storeid] NOT IN (SELECT TOP {2} [storeid] FROM [{1}stores] WHERE {3} ORDER BY {4}) AND {3} ORDER BY {4}) AS [temp1] LEFT JOIN [{1}storeranks] AS [temp2] ON [temp1].[storerid]=[temp2].[storerid] LEFT JOIN [{1}users] AS [temp3] ON [temp1].[storeid]=[temp3].[storeid]",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                condition,
        ...
        public DataTable AdminGetStoreSelectList(int pageSize, int pageNumber, string condition)
        {
            ...
            {
                if (noCondition)
                    commandText = string.Format("SELECT TOP {0} [storeid],[name] FROM [{1}stores] WHERE [storeid] < (SELECT MIN([storeid]) FROM (SELECT TOP {2} [storeid] FROM [{1}stores] ORDER BY [storeid] DESC) AS [temp1]) AS [temp2] ORDER BY [storeid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize);
                else
                    commandText = string.Format("SELECT TOP {0} [storeid],[name] FROM [{1}stores] WHERE {3} AND [storeid] < (SELECT MIN([storeid]) FROM (SELECT TOP {2} [storeid] FROM [{1}stores] WHERE {3} ORDER BY [storeid] DESC) AS [temp1]) AS [temp2] ORDER BY [storeid] DESC",
                                                pageSize,
                                                RDBSHelper.RDBSTablePre,
                                                (pageNumber - 1) * pageSize,
                                                condition);
        ...
        /// <returns></returns>
        public string AdminGetStoreListSort(string sortColumn, string sortDirection)
        {
        ...
        public void DeleteStoreById(int storeId)
        {
            ...
            string commandText = string.Format(@"DELETE FROM [{0}storeshipfees] WHERE [storestid] IN (SELECT [storestid] FROM [{0}storeshiptemplates] WHERE [storeid]=@storeid);
                                                DELETE FROM [{0}storeshiptemplates] WHERE [storeid]=@storeid;
                                                DELETE FROM [{0}storeclasses] WHERE [storeid]=@storeid;
                                                DELETE FROM [{0}storekeepers] WHERE [storeid]=@storeid;
                                                DELETE FROM [{0}stores] WHERE [storeid]=@storeid;",
                                                RDBSHelper.RDBSTablePre);
            RDBSHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
