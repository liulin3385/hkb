...
namespace BrnMall.PayPlugin.Alipay
{
    ...
    public class AlipaySubmit
    {
        ...
        private static Dictionary<string, string> BuildRequestPara(SortedDictionary<string, string> sParaTemp, string signType, string key, Encoding code)
        {
            //待签名请求参数数组
        ...
        public static string BuildRequestUrl(SortedDictionary<string, string> sParaTemp, string gateway, string inputCharset, string signType, string key, Encoding code)
        {
            //待请求参数数组
            ...
            url.AppendFormat("{0}_input_charset={1}", gateway, inputCharset);

            foreach (KeyValuePair<string, string> temp in dicPara)
            {
                url.AppendFormat("&{0}={1}", temp.Key, temp.Value);
            }
