...
var categorySelectList = "<div class='selectBoxProgressBar'><p><img src='/Admin_Mall/Content/Images/progressbar.gif'/></p></div>";
...
var brandSelectList = "<div class='selectBoxProgressBar'><p><img src='/Admin_Mall/Content/Images/progressbar.gif'/></p></div>";
...
function AjaxBrandSelectList(brandName, pageNumber) {
    ...
    $.get("/MallAdmin/Brand/SelectList?t=" + new Date(), {
        ...
        var list = "<div id='selectBrandBox'><table width='100%' ><tr><td>品牌名称：<input type='text' id='selectListSearchBrand'  name='selectListSearchBrand' style='width:120px;height:18px;'> <input type='image' onclick='SearchBrandSelectList()' src='/Admin_Mall/Content/Images/s.jpg' class='searchBut'></td></tr><tr><td><div id='selectBrandBoxCon'><ul>";
...
var productSelectList = "<div class='selectBoxProgressBar'><p><img src='/Admin_Mall/Content/Images/progressbar.gif'/></p></div>";
...
function AjaxProductSelectList(storeId, productName, pageNumber) {
    ...
    $.get("/MallAdmin/Product/ProductSelectList?t=" + new Date(), {
        ...
        var listObj = eval(data);
        var list = "<div id='selectProductBox'><table width='100%' ><tr><td>商品名称：<input type='text' id='selectListSearchProduct'  name='selectListSearchProduct' style='width:120px;height:18px;'> <input type='image' onclick='SearchProductSelectList(" + storeId + ")' src='/Admin_Mall/Content/Images/s.jpg' class='searchBut'></td></tr><tr><td><div id='selectProductBoxCon'><ul>";
        for (var i = 0; i < listObj.items.length; i++) {
        ...
        for (var j = 1; j <= listObj.count; j++) {
            if (j != listObj.page) {
                list += "<a href='javascript:;' class='bt' onclick='GoProductSelectListPage(this," + storeId + ")' pageNumber='" + j + "'>" + j + "</a>";
            }
            else {
                list += "<a href='javascript:;' onclick='GoProductSelectListPage(this," + storeId + ")' pageNumber='" + j + "'>" + j + "</a>";
            }
        }
        $.jBox.setContent(list + "</div></td></tr></table></div>");
...
/*店铺选择层开始*/
var oldSelectListSearchStore = "";
var openStoreSelectLayerBut = null;
var storeSelectList = "<div class='selectBoxProgressBar'><p><img src='/Admin_Mall/Content/Images/progressbar.gif'/></p></div>";

function SetSelectedStore(item) {
    var storeObj = $(openStoreSelectLayerBut).parent().find(".StoreId");
    storeObj.val($(item).attr("storeid"));
...
function AjaxStoreSelectList(storeName, pageNumber) {
    $.jBox.setContent(storeSelectList);
    $.get("/MallAdmin/Store/StoreSelectList?t=" + new Date(), {
        'storeName': storeName,
        'pageNumber': pageNumber
    }, function (data) {
        var listObj = eval(data);
        var list = "<div id='selectStoreBox'><table width='100%' ><tr><td>店铺名称：<input type='text' id='selectListSearchStore'  name='selectListSearchStore' style='width:120px;height:18px;'> <input type='image' onclick='SearchStoreSelectList()' src='/Admin_Mall/Content/Images/s.jpg' class='searchBut'></td></tr><tr><td><div id='selectStoreBoxCon'><ul>";
        for (var i = 0; i < listObj.items.length; i++) {
            list += "<li><a onclick='SetSelectedStore(this)' storeid='" + listObj.items[i].id + "'>" + listObj.items[i].name + "</a></li>";
        }
        list += "<div class='clear'></div></ul><div class='clear'></div></div></td></tr><tr><td><div class='page' style='position:static;'>";
        for (var j = 1; j <= listObj.count; j++) {
            if (j != listObj.page) {
                list += "<a href='javascript:;' class='bt' onclick='GoStoreSelectListPage(this)' pageNumber='" + j + "'>" + j + "</a>";
            }
            else {
                list += "<a href='javascript:;' onclick='GoStoreSelectListPage(this)' pageNumber='" + j + "'>" + j + "</a>";
            }
        }
        $.jBox.setContent(list + "</div></td></tr></table></div>");
        $("#selectListSearchStore").val(oldSelectListSearchStore);
    })
}

function SearchStoreSelectList() {
    oldSelectListSearchStore = $("#selectListSearchStore").val();
    AjaxStoreSelectList(oldSelectListSearchStore);
}

function GoStoreSelectListPage(pageObj) {
    oldSelectListSearchStore = $("#selectListSearchStore").val();
    AjaxStoreSelectList(oldSelectListSearchStore, $(pageObj).attr("pageNumber"));
}

function openStoreSelectLayer(openLayerBut) {
    $.jBox('html:storeSelectList', {
        id: 'StoreSelectLayer',
        width: 750,
        buttons: { '关闭': true },
        title: "选择店铺"
...
var shipCompanySelectList = "<div class='selectBoxProgressBar'><p><img src='/Admin_Mall/Content/Images/progressbar.gif'/></p></div>";
