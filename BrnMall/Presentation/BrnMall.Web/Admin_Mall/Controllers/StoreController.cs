...
namespace BrnMall.Web.MallAdmin.Controllers
{
    ...
    /// 商城后台店铺控制器类
    /// </summary>
    public class StoreController : BaseMallAdminController
    {
        /// <summary>
        /// 店铺列表
        /// </summary>
        public ActionResult StoreList(string storeName, string sortColumn, string sortDirection, int storeRid = 0, int storeIid = 0, int state = -1, int pageNumber = 1, int pageSize = 15)
        {
            ...
            {
                StoreList = AdminStores.AdminGetStoreList(pageModel.PageSize, pageModel.PageNumber, condition, sort),
                PageModel = pageModel,
            ...
            storeStateList.Add(new SelectListItem() { Text = "营业", Value = "0" });
            storeStateList.Add(new SelectListItem() { Text = "歇业", Value = "1" });
            storeStateList.Add(new SelectListItem() { Text = "惩罚", Value = "2" });
            storeStateList.Add(new SelectListItem() { Text = "关闭", Value = "3" });
        ...
        /// <returns></returns>
        public ContentResult StoreSelectList(string storeName, int pageNumber = 1, int pageSize = 24)
        {
        ...
        {
            if (!string.IsNullOrWhiteSpace(model.StoreName) && AdminStores.GetStoreIdByName(model.StoreName.Trim()) > 0)
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    Type = model.Type,
                    Name = model.StoreKeeperName.Trim(),
                    IdCard = model.IdCard.Trim(),
        ...
        public ActionResult EditStore(EditStoreModel model, int storeId = -1)
        {
            ...
            {
                int storeId2 = AdminStores.GetStoreIdByName(model.StoreName.Trim());
            ...
            if (ModelState.IsValid)
            {
                ...
                storeInfo.Name = model.StoreName.Trim();
                storeInfo.RegionId = model.RegionId;
                storeInfo.StoreRid = model.StoreRid;
                storeInfo.StoreIid = model.StoreIid;
                storeInfo.Logo = model.Logo == null ? "" : model.Logo.Trim();
                storeInfo.Mobile = model.Mobile == null ? "" : model.Mobile.Trim();
                storeInfo.Phone = model.Phone == null ? "" : model.Phone.Trim();
                storeInfo.QQ = model.QQ == null ? "" : model.QQ.Trim();
                storeInfo.WW = model.WW == null ? "" : model.WW.Trim();
                ...
                storeInfo.Banner = model.Banner == null ? "" : model.Banner.Trim();
                storeInfo.Announcement = model.Announcement == null ? "" : model.Announcement.Trim();
        ...
        public ViewResult DelStore(int storeId)
        {
            ...
            if (result == -1)
                return PromptView("删除失败！请先转移或删除此店铺等级下的商品！");
        ...
        public ActionResult SetStoreAdminer(StoreAdminerModel model, int storeId = -1)
        {
            ...
            {
                uid = AdminUsers.GetUidByAccountName(model.AccountName);
                if (uid < 1)
        ...
        public ActionResult StoreClassSelectList(int storeId = -1)
        {
            ...
            {
                sb.AppendFormat("{0}\"storecid\":\"{1}\",\"name\":\"{2}\"{3},", "{", storeClassInfo.StoreCid, storeClassInfo.Name, "}");
            }
        ...
        public ActionResult AddStoreClass(StoreClassModel model, int storeId = -1)
        {
            ...
            if (!string.IsNullOrWhiteSpace(model.StoreClassName) && AdminStores.GetStoreCidByStoreIdAndName(storeId, model.StoreClassName.Trim()) > 0)
        ...
        public ActionResult EditStoreClass(StoreClassModel model, int storeId = -1, int storeCid = -1)
        {
            ...
            {
                int storeCid2 = AdminStores.GetStoreCidByStoreIdAndName(storeId, model.StoreClassName.Trim());
        ...
        public ActionResult StoreShipTemplateSelectList(int storeId = -1)
        {
            ...
            {
                sb.AppendFormat("{0}\"storestid\":\"{1}\",\"title\":\"{2}\"{3},", "{", storeShipTemplateInfo.StoreSTid, storeShipTemplateInfo.Title, "}");
            }
