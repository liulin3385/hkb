...
namespace BrnMall.Web.MallAdmin.Controllers
{
    ...
    /// 商城后台日志控制器类
    /// </summary>
    public class LogController : BaseMallAdminController
    {
        /// <summary>
        /// 商城管理日志列表
        /// </summary>
        ...
        public ActionResult MallAdminLogList(string accountName, string operation, string startTime, string endTime, int pageNumber = 1, int pageSize = 15)
        {
            ...
            {
                uid = AdminUsers.GetUidByUserName(accountName);
                if (uid < 1)
                {
                    uid = -2;
                }
            ...
            PageModel pageModel = new PageModel(pageSize, pageNumber, uid == -2 ? 0 : MallAdminLogs.GetMallAdminLogCount(condition));

            MallAdminLogListModel model = new MallAdminLogListModel()
            {
                MallAdminLogList = uid == -2 ? new List<MallAdminLogInfo>() : MallAdminLogs.GetMallAdminLogList(pageModel.PageSize, pageModel.PageNumber, condition),
                PageModel = pageModel,
        ...
        /// <param name="pageSize">每页数</param>
        /// <returns></returns>
        public ActionResult CreditLogList(string userName, string startTime, string endTime, int pageNumber = 1, int pageSize = 15)
        {
            int uid = 0;
            if (!string.IsNullOrWhiteSpace(userName))
            {
                uid = AdminUsers.GetUidByUserName(userName);
                if (uid < 1)
                    uid = 0;
            }
            ...
            MallUtils.SetAdminRefererCookie(string.Format("{0}?pageNumber={1}&pageSize={2}&userName={3}&startTime={4}&endTime={5}",
