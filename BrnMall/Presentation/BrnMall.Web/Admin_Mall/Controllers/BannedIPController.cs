...
namespace BrnMall.Web.MallAdmin.Controllers
{
    ...
    /// 商城后台禁止IP控制器类
    /// </summary>
    public class BannedIPController : BaseMallAdminController
    {
        /// <summary>
        /// 禁止IP列表
        /// </summary>
        ...
        public ActionResult Add(BannedIPModel model)
        {
            ...
            if (AdminBannedIPs.GetBannedIPIdByIP(ip.Trim()) > 0)
        ...
        public ActionResult Edit(BannedIPModel model, int id = -1)
        {
            ...
            int id2 = AdminBannedIPs.GetBannedIPIdByIP(ip.Trim());
            if (id2 > 0 && id2 != id)
                ModelState.AddModelError("IP4", "IP已经存在");

            if (ModelState.IsValid)
            {
                bannedIPInfo.IP = ip.Trim();
