...
namespace BrnMall.Web.MallAdmin.Controllers
{
    ...
    /// 商城后台商城管理员组控制器类
    /// </summary>
    public class MallAdminGroupController : BaseMallAdminController
    {
        /// <summary>
        /// 商城管理员组列表
        /// </summary>
        ...
        {
            if (MallAdminGroups.GetMallAdminGroupIdByTitle(model.AdminGroupTitle.Trim()) > 0)
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    Title = model.AdminGroupTitle.Trim(),
        ...
        public ActionResult Edit(MallAdminGroupModel model, int mallAGid = -1)
        {
            ...
            int mallAGid2 = MallAdminGroups.GetMallAdminGroupIdByTitle(model.AdminGroupTitle.Trim());
            if (mallAGid2 > 0 && mallAGid2 != mallAGid)
                ModelState.AddModelError("AdminGroupTitle", "名称已经存在");

            if (ModelState.IsValid)
            {
                mallAdminGroupInfo.Title = model.AdminGroupTitle.Trim();
        ...
        public ActionResult Del(int mallAGid = -1)
        {
            ...
            if (result == -1)
                return PromptView("删除失败！请先转移或删除此商城管理员组下的用户！");
            else if (result == -2)
