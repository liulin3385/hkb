...
namespace BrnMall.Web.MallAdmin.Controllers
{
    ...
    /// 商城后台用户等级控制器类
    /// </summary>
    public class UserRankController : BaseMallAdminController
    {
        /// <summary>
        /// 会员等级列表
        /// </summary>
        ...
        {
            if (AdminUserRanks.GetUserRidByTitle(model.UserRankTitle.Trim()) > 0)
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    System = 0,
                    Title = model.UserRankTitle.Trim(),
                    Avatar = model.Avatar,
        ...
        public ActionResult Edit(UserRankModel model, int userRid = -1)
        {
            ...
            if (ModelState.IsValid)
            {
                userRankInfo.Title = model.UserRankTitle.Trim();
        ...
        public ActionResult Del(int userRid = -1)
        {
            ...
            if (result == -1)
                return PromptView("删除失败！请先转移或删除此会员等级下的用户！");
            else if (result == -2)
