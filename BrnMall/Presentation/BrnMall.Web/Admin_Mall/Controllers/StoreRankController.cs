...
namespace BrnMall.Web.MallAdmin.Controllers
{
    ...
    /// 商城后台店铺等级控制器类
    /// </summary>
    public class StoreRankController : BaseMallAdminController
    {
        /// <summary>
        /// 店铺等级列表
        /// </summary>
        ...
        {
            if (!string.IsNullOrWhiteSpace(model.RankTitle) && AdminStoreRanks.GetStoreRidByTitle(model.RankTitle.Trim()) > 0)
        ...
        public ActionResult Edit(StoreRankModel model, int storeRid = -1)
        {
            ...
            if (!string.IsNullOrWhiteSpace(model.RankTitle))
            {
                int storeRid2 = AdminStoreRanks.GetStoreRidByTitle(model.RankTitle.Trim());
            ...
            if (ModelState.IsValid)
            {
                storeRankInfo.Title = model.RankTitle.Trim();
        ...
        public ActionResult Del(int storeRid = -1)
        {
            ...
            if (result == -1)
                return PromptView("删除失败！请先转移或删除此店铺等级下的店铺！");
        ...
        private void Load()
        {
            ...
            string[] sizeList = StringHelper.SplitString(WorkContext.MallConfig.StoreRankAvatarThumbSize);
