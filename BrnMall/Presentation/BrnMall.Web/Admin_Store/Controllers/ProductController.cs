...
namespace BrnMall.Web.StoreAdmin.Controllers
{
    ...
    /// 店铺后台商品控制器类
    /// </summary>
    public class ProductController : BaseStoreAdminController
    {
        /// <summary>
        /// 在售商品列表
        /// </summary>
        ...
        /// <returns></returns>
        public ActionResult OnSaleProductList(string productName, string categoryName, string brandName, string sortColumn, string sortDirection, int storeCid = -1, int cateId = -1, int brandId = -1, int pageNumber = 1, int pageSize = 15)
        {
            ...
            MallUtils.SetAdminRefererCookie(string.Format("{0}?pageNumber={1}&pageSize={2}&sortColumn={3}&sortDirection={4}&storeCid={5}&productName={6}&cateId={7}&brandId={8}&categoryName={9}&brandName={10}",
        ...
        /// <param name="pageSize">每页数</param>
        /// <returns></returns>
        public ActionResult OutSaleProductList(string productName, string categoryName, string brandName, string sortColumn, string sortDirection, int storeCid = -1, int cateId = -1, int brandId = -1, int pageNumber = 1, int pageSize = 15)
        {
        ...
        /// <param name="pageSize">每页数</param>
        /// <returns></returns>
        public ActionResult RecycleBinProductList(string productName, string categoryName, string brandName, string sortColumn, string sortDirection, int storeCid = -1, int cateId = -1, int brandId = -1, int pageNumber = 1, int pageSize = 15)
        {
        ...
        {
            List<SelectListItem> storeClassList = new List<SelectListItem>();
            storeClassList.Add(new SelectListItem() { Text = "选择分类", Value = "-1" });
            foreach (StoreClassInfo storeClassInfo in AdminStores.GetStoreClassList(WorkContext.StoreId).FindAll(x => x.HasChild == 0))
            {
                storeClassList.Add(new SelectListItem() { Text = storeClassInfo.Name, Value = storeClassInfo.StoreCid.ToString() });
            }
        ...
        public ActionResult AddProductImage(string showImg, int displayOrder = 0, int pid = -1)
        {
            PartProductInfo partProductInfo = AdminProducts.AdminGetPartProductById(pid);
            if (partProductInfo == null)
                return View("Prompt", new PromptModel("商品不存在！"));
            if (partProductInfo.StoreId != WorkContext.StoreId)
                return PromptView("不能操作其它店铺的商品");
        ...
        public ActionResult DelProductImage(int pImgId = -1)
        {
            ...
            if (productImageInfo == null)
                return Content("0");
            PartProductInfo partProductInfo = AdminProducts.AdminGetPartProductById(productImageInfo.Pid);
            if (partProductInfo == null || partProductInfo.StoreId != WorkContext.StoreId)
                return Content("0");
        ...
        public ActionResult AddProductKeyword(string keyword, int relevancy = 0, int pid = -1)
        {
            ...
            AddStoreAdminLog("添加商品关键词", "添加商品关键词,商品ID为:" + pid);
        ...
        {
            int pid = AdminProducts.GetPidByKeywordId(keywordId);
            PartProductInfo partProductInfo = AdminProducts.AdminGetPartProductById(pid);
            if (partProductInfo == null || partProductInfo.StoreId != WorkContext.StoreId)
                return Content("0");
            ...
            if (result)
            {
                AddStoreAdminLog("修改商品关键词的相关性", "修改商品关键词的相关性,商品关键词ID:" + keywordId);
                return Content("1");
            }
            else
            {
        ...
        {
            if (!AdminProducts.IsStoreProductByKeywordId(WorkContext.StoreId, keywordIdList))
                return Content("0");

            bool result = AdminProducts.DeleteProductKeyword(keywordIdList);
            if (result)
            {
                AddStoreAdminLog("删除商品关键词", "删除商品关键词,商品关键词ID:" + CommonHelper.IntArrayToString(keywordIdList));
                return Content("1");
            }
            else
            {
