...
namespace BrnMall.Web.StoreAdmin.Controllers
{
    ...
    /// 店铺后台商品咨询控制器类
    /// </summary>
    public class ProductConsultController : BaseStoreAdminController
    {
        /// <summary>
        /// 商品咨询列表
        /// </summary>
        public ActionResult ProductConsultList(string accountName, string productName, string consultMessage, string consultStartTime, string consultEndTime, string sortColumn, string sortDirection, int pid = -1, int consultTypeId = -1, int pageNumber = 1, int pageSize = 15)
        {
            ...
            if (!string.IsNullOrWhiteSpace(accountName))
                uid = AdminUsers.GetUidByAccountName(accountName);

            string condition = AdminProductConsults.AdminGetProductConsultListCondition(WorkContext.StoreId, pid, uid, consultMessage, consultStartTime, consultEndTime);
