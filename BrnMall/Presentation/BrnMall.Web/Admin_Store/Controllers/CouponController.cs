...
namespace BrnMall.Web.StoreAdmin.Controllers
{
    ...
    /// 店铺后台优惠劵控制器类
    /// </summary>
    public class CouponController : BaseStoreAdminController
    {
        /// <summary>
        /// 优惠劵类型列表
        /// </summary>
        ...
        public ActionResult AddCouponType(CouponTypeModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                CouponTypeInfo couponTypeInfo = new CouponTypeInfo()
                {
                    ...
                    Name = model.CouponTypeName.Trim(),
                    Money = model.Money,
                    Count = model.Count,
        ...
        /// <param name="pageSize">每页数</param>
        /// <returns></returns>
        public ActionResult CouponList(string sn, string userName, int couponTypeId = -1, int pageNumber = 1, int pageSize = 15)
        {
            CouponTypeInfo couponTypeInfo = AdminCoupons.AdminGetCouponTypeById(couponTypeId);
            if (couponTypeInfo == null)
                return PromptView("优惠劵类型不存在！");
            ...
            MallUtils.SetAdminRefererCookie(string.Format("{0}?pageNumber={1}&pageSize={2}&couponTypeId={3}&sn={4}&userName={5}",
                                                           Url.Action("CouponList"),
                                                           pageModel.PageNumber, pageModel.PageSize,
                                                           couponTypeId, sn, userName));
            return View(model);
        }
        ...
        public ActionResult SendCoupon(SendCouponModel model, int couponTypeId = -1)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    int uid = Users.GetPartUserByName(model.UValue).Uid;
                    AdminCoupons.AdminSendCouponToUser(uid, couponTypeId, WorkContext.StoreId, couponTypeInfo.Money, model.Count, WorkContext.Uid, DateTime.Now, WorkContext.IP);
                    AddStoreAdminLog("按用户名发放优惠劵", string.Format("用户名:{0},优惠劵类型id:{1},名称:{2},发放数量:{3}", model.UValue, couponTypeId, couponTypeInfo.Name, model.Count));
                }
                else
                {
