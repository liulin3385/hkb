...
function selectAttrValue(obj) {
    ...
    if (productPageType == 1) {
        ...
        if (radioObj.attr("checked") == false) {
            changeproductattributeurl = "/StoreAdmin/product/DelProductAttribute?pid=" + pid + "&attrId=" + radioObj.parent().find("input[name=AttrIdList]").val() + "&t=" + new Date();
        }
        else {
            if (isInput == "0") {
                changeproductattributeurl = "/StoreAdmin/product/UpdateProductAttribute?type=0" + "&pid=" + pid + "&attrId=" + radioObj.parent().find("input[name=AttrIdList]").val() + "&attrValueId=" + radioObj.val() + "&t=" + new Date();
            }
        }
...
function SetSelectedCategory(selectedCateId, selectedCateName) {
    ...
    if (selectedCateId != "-1") {
        $.get("/StoreAdmin/Category/AANDVJSONList?cateId=" + selectedCateId + "&time=" + new Date(), function (data) {
...
$(function () {
    ...
    if (productPageType != 2 && $("#CateId").val() > 0) {
        $("#categoryTable").find("tr:not('.keepTr')").remove();
        $.get("/StoreAdmin/Category/AANDVJSONList?cateId=" + $("#CateId").val() + "&time=" + new Date(), function (data) {
            result = eval(data);
            var optionList = buildAttrTable(result);
            $("#categoryTable").prepend(optionList);
                ...
                if (productPageType == 1) {
                    ...
                    $(".inputattrvalue").blur(function () {
                        ...
                        if (inputattrvalueobj.val() != tempinputattrvalue) {
                            $.jBox.tip("正在更新...", 'loading');
                            $.get("/StoreAdmin/product/UpdateProductAttribute?type=1" + "&pid=" + pid + "&attrId=" + inputattrvalueobj.parent().find("input[name=AttrIdList]").val() + "&attrValueId=" + inputattrvalueobj.attr("attrValueIdSign") + "&inputValue=" + encodeURIComponent(inputattrvalueobj.val()) + "&t=" + new Date(), function (data, textStatus) {
                                if (data != "0") {
                                    $.jBox.tip('更新成功！', 'success');
                                } else {
