    ...
    public class SendCouponModel : IValidatableObject
    {
        ...
        public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
        {
            ...
            else if (UType == 1)
            {
                ...
                if (string.IsNullOrWhiteSpace(UValue))
                {
                    errorList.Add(new ValidationResult("请输入用户名!", new string[] { "UValue" }));
                }
                else
                {
                    if (!AdminUsers.IsExistUserName(UValue))
                        errorList.Add(new ValidationResult("用户名不存在!", new string[] { "UValue" }));
                }
            }
